#include "Pure_TCPServer.h"

NS_NET_UV_BEGIN

enum 
{
	Pure_TCP_SVR_OP_STOP_SERVER,	// 停止服务器
	Pure_TCP_SVR_OP_SEND_DATA,	// 发送消息给某个会话
	Pure_TCP_SVR_OP_DIS_SESSION,	// 断开某个会话
	Pure_TCP_SVR_OP_SEND_DIS_SESSION_MSG_TO_MAIN_THREAD,//向主线程发送会话已断开
};


////////////////////////////////////////////////////////////////////////////////////////////////////////////

Pure_TCPServer::Pure_TCPServer()
	: m_start(false)
	, m_server(NULL)
	, m_sessionID(0)
{
...
bool Pure_TCPServer::startServer(const char* ip, uint32_t port, bool isIPV6, int32_t maxCount)
{
...
bool Pure_TCPServer::stopServer()
{
	if (!m_start)
		return false;
	m_start = false;
	pushOperation(Pure_TCP_SVR_OP_STOP_SERVER, NULL, NULL, 0U);
	return true;
}
...
void Pure_TCPServer::updateFrame()
{
	...
	while (!m_msgDispatchQue.empty())
	{
		...
		case NetThreadMsgType::DIS_CONNECT:
		{
			...
			pushOperation(Pure_TCP_SVR_OP_SEND_DIS_SESSION_MSG_TO_MAIN_THREAD, NULL, 0, Msg.pSession->getSessionID());
		}break;
...
void Pure_TCPServer::send(uint32_t sessionID, char* data, uint32_t len)
{
	if (data == NULL || len <= 0)
		return;
...
{
	pushOperation(Pure_TCP_SVR_OP_DIS_SESSION, NULL, 0, sessionID);
}

void Pure_TCPServer::run()
{
	startIdle();
...
void Pure_TCPServer::onNewConnect(uv_stream_t* server, int32_t status)
{
	TCPSocket* client = m_server->accept(server, status);
	if (client != NULL)
	{
		Pure_TCPSession* session = Pure_TCPSession::createSession(this, client);
		if (session == NULL)
		{
		...
		{
			session->setSessionRecvCallback(std::bind(&Pure_TCPServer::onSessionRecvData, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
			session->setSessionClose(std::bind(&Pure_TCPServer::onSessionClose, this, std::placeholders::_1));
...
void Pure_TCPServer::onServerSocketClose(Socket* svr)
{
	m_serverStage = ServerStage::CLEAR;
}

void Pure_TCPServer::startFailureLogic()
{
	m_server->~TCPSocket();
	fc_free(m_server);
	m_server = NULL;
...
void Pure_TCPServer::addNewSession(Pure_TCPSession* session)
{
	if (session == NULL)
	{
		return;
...
void Pure_TCPServer::onSessionClose(Session* session)
{
	if (session == NULL)
	{
		return;
...
void Pure_TCPServer::onSessionRecvData(Session* session, char* data, uint32_t len)
{
	pushThreadMsg(NetThreadMsgType::RECV_DATA, session, data, len);
}

void Pure_TCPServer::executeOperation()
{
	...
	while (!m_operationDispatchQue.empty())
	{
		...
		{
		case Pure_TCP_SVR_OP_SEND_DATA :		// 数据发送
		{
		...
		case Pure_TCP_SVR_OP_DIS_SESSION:	// 断开连接
		{
			auto it = m_allSession.find(curOperation.sessionID);
			if (it != m_allSession.end())
			{
		...
		case Pure_TCP_SVR_OP_SEND_DIS_SESSION_MSG_TO_MAIN_THREAD:
		{
			auto it = m_allSession.find(curOperation.sessionID);
			if (it != m_allSession.end())
			{
		...
		case Pure_TCP_SVR_OP_STOP_SERVER:
		{
...
void Pure_TCPServer::clearData()
{
	...
	while (!m_operationQue.empty())
	{
		if (m_operationQue.front().operationType == Pure_TCP_SVR_OP_SEND_DATA)
		{
